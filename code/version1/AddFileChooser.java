...
//构造函数传递两个子菜单项，便于对其加上监听
	...
	//加载文件的方法单独列出
	public static void loadFile(File file) throws IOException {//因拖入功能要调用此函数，故设static使其为类函数
		/*
		...
		if (file.getName().endsWith(".md")) {
			...
			ThemeLabel themeLabel = new ThemeLabel(0, 0);
			themeLabel.setText(content.substring(2));
			//new ThemeDetect(MainWindow.pan).addConnect(themeLabel);
			themeLabel.setRank(themeLabel.getRank());

			MainWindow.pan.setrootThemeLabel(themeLabel);
			//MainWindow.pan.getrootThemeLabel().getMyThemeFrame().setVisible(true);
